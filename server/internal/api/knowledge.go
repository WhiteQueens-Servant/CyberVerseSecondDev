...
func (r *Router) indexKnowledgeSource(ctx context.Context, characterID, sourceID string) error {
	...
	charDir := r.charStore.CharDir(characterID)
	log.Printf("knowledge index: characterID=%s charDir=%s sourcePath=%s", characterID, charDir, path)
	chunkCount, err := r.orch.IndexKnowledgeSource(ctx, characterID, charDir, source, path)
	if err != nil {
		_, _ = r.ragStore.MarkFailed(characterID, sourceID, err)
		return err
	}
