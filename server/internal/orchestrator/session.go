...
type Session struct {
	...
	// dbSyncedCount tracks how many History messages have been written to cyberverse.db.
	// Used by persistSessionConversation to avoid duplicate inserts on repeated calls.
	dbSyncedCount int    `json:"-"`
	dbSessionID   string `json:"-"` // session ID in cyberverse.db (set on first sync)
	mu            sync.RWMutex
}
...
// DBSessionID returns the session ID in cyberverse.db (set by syncSessionToDB on first persist).
func (s *Session) DBSessionID() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.dbSessionID
}
