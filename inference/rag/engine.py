...
class RAGEngine:
    ...
    def _resolve_source_path(self, source_path: str) -> Path:
        ...
        if path.exists():
            ...
            logger.info("RAG source path resolved directly: %s -> %s", source_path, resolved)
            return resolved
        ...
        if data_relative is not None:
            for data_root in self._data_root_candidates():
                ...
                if candidate.exists():
                    ...
                    logger.info("RAG source path resolved via data root: %s -> %s", source_path, resolved)
                    return resolved

        resolved = path.resolve()
        logger.warning("RAG source path not found: %s -> %s (file may not exist)", source_path, resolved)
        return resolved
    ...
    def _embedding_model(self):
        ...
        kwargs: dict[str, Any] = {
            "model": model,
            # DashScope (Qwen) embedding API only accepts str or list[str],
            # not token IDs. Disable langchain's pre-tokenization to prevent
            # sending integer token lists that DashScope rejects with 400.
    ...
    def _vector_store(self, character_id: str, character_dir: str):
        ...
        logger.info("RAG vector store: character_dir=%s, persist_dir=%s", character_dir, persist_dir)
    ...
    def _index_source_sync(self, req: RAGIndexRequest) -> int:
        ...
        logger.info(
            "RAG indexing: character_id=%s, character_dir=%s, source_id=%s, source_path=%s",
            req.character_id, req.character_dir, req.source_id, req.source_path,
        )
