...
function launch() {
  router.push(`/admin/launch/${props.character.id}`)
}

function edit() {
  router.push(`/admin/characters/${props.character.id}/edit`)
}
</script>

<template>
  <div class="group relative min-h-[340px] flex flex-col justify-end bg-cv-surface border border-cv-border rounded-cv-lg overflow-hidden hover:border-cv-accent hover:shadow-[0_0_20px_rgba(107,175,141,0.15)] hover:-translate-y-0.5 transition-all duration-200 cursor-pointer"
       @click="edit">
    <!-- Avatar image -->
