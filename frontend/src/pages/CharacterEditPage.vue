...
import type { CharacterComponents, CharacterForm, ComponentOption, ComponentsResponse, ImageInfo, ScenicRoute } from '../types'
import { OPENAI_VOICE_OPTIONS, QWEN_OMNI_VOICE_OPTIONS, QWEN_TTS_VOICE_OPTIONS, VOICE_OPTIONS } from '../types'
import { uploadAvatar, getCharacterImages, deleteCharacterImage, activateCharacterImage, testCharacterVoice, getComponents, listRoutes } from '../services/api'
...
const form = ref<CharacterForm>({
  ...
  scenic_category: '',
  recommended_routes: [],
  scenic_system_prompt: '',
})

const availableRoutes = ref<ScenicRoute[]>([])
...
onMounted(async () => {
  ...
  // Load available routes for recommended_routes selector
  try {
    availableRoutes.value = await listRoutes()
  } catch (e) {
    ...
    if (store.current) {
      ...
      try {
        form.value = {
          ...
          tags: [...c.tags],
          scenic_category: c.scenic_category || '',
          recommended_routes: [...(c.recommended_routes || [])],
          scenic_system_prompt: c.scenic_system_prompt || '',
        }
  ...
  <div class="min-h-screen bg-cv-base flex flex-col">
    ...
    <main class="flex-1 max-w-[1100px] mx-auto w-full px-12 pb-24 flex gap-8">
      ...
      <div class="flex-1 flex flex-col gap-6">
        ...
        <!-- Section 3b: Scenic configuration -->
        <section class="bg-cv-surface border border-cv-border rounded-cv-lg p-6">
          <h2 class="text-base font-semibold text-cv-text mb-5">{{ t('characterEdit.scenicConfig') }}</h2>

          <label class="block mb-4">
            <span class="text-[13px] font-medium text-cv-text-secondary">{{ t('characterEdit.scenicCategory') }}</span>
            <select v-model="form.scenic_category"
                    class="mt-1.5 w-full h-[42px] bg-cv-elevated border border-cv-border rounded-cv-md px-4 text-sm text-cv-text focus:border-cv-accent focus:outline-none focus:shadow-[0_0_0_2px_rgba(59,130,246,0.15)] transition-all">
              <option value="">{{ t('characterEdit.scenicCategoryPlaceholder') }}</option>
              <option value="guide">{{ t('characterEdit.scenicCategoryGuide') }}</option>
              <option value="narrator">{{ t('characterEdit.scenicCategoryNarrator') }}</option>
              <option value="service">{{ t('characterEdit.scenicCategoryService') }}</option>
            </select>
            <p class="text-[11px] text-cv-text-muted mt-1">{{ t('characterEdit.scenicCategoryHint') }}</p>
          </label>

          <label class="block mb-4">
            <span class="text-[13px] font-medium text-cv-text-secondary">{{ t('characterEdit.scenicSystemPrompt') }}</span>
            <textarea v-model="form.scenic_system_prompt"
                      :placeholder="t('characterEdit.scenicSystemPromptPlaceholder')"
                      class="mt-1.5 w-full h-28 bg-cv-elevated border border-cv-border rounded-cv-md px-4 py-3 text-[13px] text-cv-text placeholder:text-cv-text-muted resize-y leading-[22px] focus:border-cv-accent focus:outline-none focus:shadow-[0_0_0_2px_rgba(59,130,246,0.15)] transition-all" />
            <p class="text-[11px] text-cv-text-muted mt-1">{{ t('characterEdit.scenicSystemPromptHint') }}</p>
          </label>

          <div>
            <span class="text-[13px] font-medium text-cv-text-secondary">{{ t('characterEdit.recommendedRoutes') }}</span>
            <p class="text-[11px] text-cv-text-muted mb-2">{{ t('characterEdit.recommendedRoutesHint') }}</p>
            <div v-if="availableRoutes.length === 0" class="text-sm text-cv-text-muted py-3">
              {{ t('characterEdit.noRoutes') }}
            </div>
            <div v-else class="space-y-2 max-h-48 overflow-y-auto">
              <label v-for="route in availableRoutes" :key="route.id"
                     class="flex items-start gap-3 p-3 bg-cv-elevated border border-cv-border rounded-cv-md cursor-pointer hover:border-cv-accent transition-colors">
                <input type="checkbox" :value="route.id" v-model="form.recommended_routes"
                       class="mt-0.5 accent-cv-accent" />
                <div class="flex-1 min-w-0">
                  <span class="text-sm font-medium text-cv-text">{{ route.name }}</span>
                  <div class="flex items-center gap-2 mt-0.5">
                    <span v-if="route.duration" class="text-[11px] text-cv-text-muted">{{ route.duration }}</span>
                    <span v-if="route.difficulty" class="text-[11px] text-cv-text-muted">· {{ route.difficulty }}</span>
                  </div>
                </div>
              </label>
            </div>
