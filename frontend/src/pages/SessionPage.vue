...
const isScenicMode = computed(() => launchState.value?.return_path === '/scenic')
...
async function handleDisconnect() {
  ...
  webrtcDisconnect()
  chatDisconnect()
  const returnPath = launchState.value?.return_path || '/admin/characters'
  if (sessionId.value) {
...
<template>
  <div class="session-page" :class="{ 'chat-collapsed': isChatCollapsed, 'scenic-mode': isScenicMode }">
    <!-- Left: Video area (60%) -->
    ...
    <div class="session-chat-sidebar" :aria-hidden="isChatCollapsed" :inert="isChatCollapsed">
      <div class="session-chat-inner">
        ...
        <div class="h-[52px] shrink-0 flex items-center justify-between px-5 border-b border-cv-border-subtle">
          ...
          <button class="text-[13px] text-cv-text-muted hover:text-cv-text transition-colors cursor-pointer" @click="clearMessages">{{ t('session.clear') }}</button>
        </div>
...
/* Scenic mode: light sidebar + light control bar */
.scenic-mode .session-chat-sidebar {
  background: #f8faf8;
  border-left-color: #d4e8d4;
}

.scenic-mode .session-chat-inner > div:first-child {
  border-bottom-color: #d4e8d4;
}

.scenic-mode .session-chat-inner > div:first-child span {
  color: #1a1a1a;
}

.scenic-mode .session-chat-inner > div:first-child button {
  color: #6b7280;
}

.scenic-mode .session-chat-inner > div:first-child button:hover {
  color: #1a1a1a;
  background: #f0fdf4;
}

.scenic-mode .session-chat-inner > div:last-child span {
  color: #9ca3af;
}

.scenic-mode .chat-toggle-button {
  color: #6b7280;
}

.scenic-mode .chat-toggle-button:hover {
  color: #1a1a1a;
  background: #f0fdf4;
}

.scenic-mode .chat-expand-button {
  color: #1a1a1a;
...
.scenic-mode .chat-expand-button:hover {
  background: rgba(255, 255, 255, 0.95);
}

/* Control bar: semi-transparent light */
.scenic-mode :deep(.absolute.bottom-6) {
  background: rgba(255, 255, 255, 0.88);
  border-color: rgba(22, 163, 74, 0.2);
  box-shadow: 0 4px 16px rgba(0, 0, 0, 0.08);
}

.scenic-mode :deep(.absolute.bottom-6 button.bg-white\/10) {
  background: rgba(22, 163, 74, 0.1);
  color: #374151;
}

.scenic-mode :deep(.absolute.bottom-6 button.bg-white\/10:hover) {
  background: rgba(22, 163, 74, 0.18);
}

.scenic-mode :deep(.absolute.bottom-6 .bg-cv-accent) {
  background: #16a34a;
  box-shadow: 0 2px 8px rgba(22, 163, 74, 0.3);
}

.scenic-mode :deep(.absolute.bottom-6 .text-\[11px\]) {
  color: #6b7280;
}

.scenic-mode :deep(.absolute.bottom-6 .bg-cv-success) {
  background: #16a34a;
}

.scenic-mode :deep(.absolute.bottom-6 .bg-cv-danger) {
  background: #ef4444;
}

/* Back button: light style */
.scenic-mode :deep(.absolute.top-5.left-5) {
  background: rgba(255, 255, 255, 0.85);
  color: #1a1a1a;
  border: 1px solid #d4e8d4;
}

.scenic-mode :deep(.absolute.top-5.left-5:hover) {
  background: rgba(255, 255, 255, 0.95);
}

/* FPS button: light glass */
.scenic-mode :deep(.absolute.top-5.right-5) {
  background: rgba(255, 255, 255, 0.85);
  color: #374151;
  border: 1px solid #d4e8d4;
}

.scenic-mode :deep(.absolute.top-5.right-5:hover) {
  background: rgba(255, 255, 255, 0.95);
}
</style>
